    ...
    return (
        <>
            ...
            <nav className="w-full flex justify-between items-center py-5 px-10 fixed top-0 z-50">
                <a href="#top">
                    <Image src={assets.logo} alt="logo-personal" className="w-28 cursor-pointer mr-14" width={112} height={40} />
                </a>
                <ul className="hidden md:flex space-x-10 items-center gap-6 lg:gap-8 rounded-full px-12 py-3 bg-white shadow-md bg-opacity-50">
                    {["Home", "About me", "Services", "My Work", "Contact me"].map((item, index) => (
                        <li key={index}>
                            <a className="font-Ovo" href={`#${item.toLowerCase().replace(/\s+/g, '')}`}>{item}</a>
                        </li>
                    ))}
                </ul>
                <div className="flex items-center gap-4">
                    ...
                    <a href="#contact" className="hidden lg:flex items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4 font-Ovo">
                        Contact <Image src={assets.arrow_icon} alt="contact" className="w-3" width={12} height={12} />
                    </a>
                ...
                {/* Mobile menu */}
                <ul ref={sideMenuRef} className="flex md:hidden flex-col items-center gap-4 py-20 px-10 fixed right-[-16rem] top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition-transform duration-500">
                    ...
                    {["Home", "About me", "Services", "My Work", "Contact me"].map((item, index) => (
                        <li key={index}>
                            <a className="font-Ovo" onClick={closeMenu} href={`#${item.toLowerCase().replace(/\s+/g, '')}`}>{item}</a>
                        </li>
                    ))}
                </ul>
            </nav>
